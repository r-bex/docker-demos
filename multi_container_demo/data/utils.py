...
def connect_to_kafka(connection_type, address, topic=None, logger=None):
    ...
    while not connected_to_kafka:
        try:
            logger.info("Connecting to kafka @ {} attempt #{}".format(
                address, str(attempted_connections)))
            ...
            if attempted_connections < 5:
                ...
                logger.warning(
                    "Failed to connect to kafka at '{}', retrying in 5 seconds (attempt {})"
                    .format(address, attempted_connections))
                logger.warning(str(e))
                time.sleep(5)
            else:
                logger.error(
                    "Failed to connect to kafka at '{}' after {} retries, exiting"
                    .format(address, attempted_connections))
                break
